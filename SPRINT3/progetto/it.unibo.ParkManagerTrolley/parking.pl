...
pickup(TOKEN,X) :- retract(occupied(TOKEN,_,X)), unoccupySlot(X).
...
timedout(CURRENTIME,[H|T]) :- occupied(0,TIME,H), timemax(TMAX),  CURRENTIME > TIME + TMAX, pickup(0,H), timedout(CURRENTIME,T).
